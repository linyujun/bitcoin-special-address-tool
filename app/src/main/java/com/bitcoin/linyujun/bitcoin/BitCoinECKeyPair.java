...
import com.bitcoin.linyujun.Base58Check;
...
import com.bitcoin.linyujun.base.Key;
...
public class BitCoinECKeyPair extends ECKeyPair {
    ...
    @Override
    public BTCTransaction sign(byte[] unSignByte) throws ValidationException {
        try {
            BTCTransaction btcTransaction = new BTCTransaction(unSignByte);
            //签名交易
    ...
    public static BTCTransaction.Input sign(BTCTransaction transaction, int index, BigInteger
        ...
        //清空无关Input的Script，做hash的时候只需要对应的锁定脚本
        signatureForm(transaction, index, publicScript);

        //对交易进行双 Sha256 并且使用私钥 签名 ，结果作为输入签名脚本
     ...
     * @return 签名
     */
    public static String mkPubKeyScript(String address) {
        //设置加密格式
